...
-- Define custom autocommand group and helpers to create an autocommand.
...
_G.Config.new_buf_autocmd = function(event, buffer, callback, desc)
    local opts = { group = gr, buffer = buffer, callback = callback, desc = desc }
    vim.api.nvim_create_autocmd(event, opts)
end
